...
export class Funcion implements IFuncion {
  ...
  public crearFuncion(arg: IFuncionArgs): TFuncion {
    ...
    if (arg.eventos) {
      for (const evento of arg.eventos) {
        switch (evento.tipo) {
          case 's3':
            this.crearEventoS3(funcion, evento.parametros as IEventoS3, nombreRecursoFuncion);
            break;
          default:
  ...
  private crearEventoS3(funcion: TFuncion, eventoS3: IEventoS3, nombreRecursoFuncion: string) {
    ...
    const permisosS3 = new aws.lambda.Permission(`${PREF_LAMBPERMISSION}${nombreRecursoFuncion}${eliminarCaracteresEspeciales(eventoS3.nombreBucket)}`, {
    ...
    new aws.s3.BucketNotification(`${PREF_S3NOTIFICATION}${eliminarCaracteresEspeciales(eventoS3.nombreBucket)}${nombreRecursoFuncion}`, {
